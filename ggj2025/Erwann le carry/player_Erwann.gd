...
func _physics_process(delta: float) -> void:
	...
	var joy_vector = Vector2(Input.get_joy_axis(player_index, JOY_AXIS_LEFT_X), Input.get_joy_axis(player_index, JOY_AXIS_LEFT_Y))
	_velocity += SPEED * joy_vector * delta
